...
(function () {
  ...
  if (CFG.builder) (function () {
    ...
    function renderPreview() {
      ...
      var pool = names.filter(function (pp) { return pp.t; }); /* trophy deals only (Avi) */
      pool.slice(0, MAXP).forEach(function (pp) {
        var pill = document.createElement("span");
        pill.className = "cansw-pill";
      ...
      var shown = Math.min(pool.length, MAXP);
      var hiddenN = names.length - shown; /* chip counts the whole view, not just trophy holders */
      var more = null;
      if (shown && hiddenN > 0) {
      ...
      /* top deals for this view: biggest savers, clickable straight into the picker */
      var ranked = BP
