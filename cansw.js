...
(function () {
  ...
  /* ---- email-gated budget builder (CFG.builder) ---- */
  if (CFG.builder) (function () {
    ...
    var reopen = document.getElementById("canswGateReopen");
    var builder = document.getElementById("canswBuilder");
    if (!gate || !builder) return; /* stale markup on this surface; degrade to phase 1 */
    var catsEl = document.getElementById("canswBCats");
    var listEl = document.getElementById("canswBList");
    var legendEl = document.getElementById("canswBLegend");
    ...
    var BCAT_NAMES = Object.keys(BCATS).sort();
    var bCat = "All";
    ...
    function renderCats() {
      catsEl.innerHTML = "";
      ["All"].concat(BCAT_NAMES).forEach(function (c) {
        var b = document.createElement("button");
        b.className = "cansw-b-cat";
        b.type = "button";
        b.textContent = c === "All" ? "All partners" : c;
        b.setAttribute("aria-pressed", String(bCat === c));
        b.addEventListener("click", function () { bCat = c; openRow = null; renderCats(); renderList(); });
    ...
    function rowFor(p) {
      ...
      row.className = "cansw-b-row";
      var head = document.createElement("div");
      head.className = "cansw-b-row-head";
      head.appendChild(logoEl(p.n));
      var info = document.createElement("div");
      info.className = "cansw-b-row-info";
      var nm = document.createElement("p");
      nm.className = "cansw-b-name";
      nm.innerHTML = esc(p.n) + (p.t ? " <span class='cansw-trophy' title='Exclusive to CAN or the best deal this partner offers'>🏆</span>" : "");
      ...
      btn.className = "cansw-b-add";
      btn.type = "button";
      if (inCart(p.n) > -1) {
        btn.disabled = true;
        btn.textContent = "✓";
        btn.setAttribute("aria-label", p.n + " added to your budget");
      } else {
        btn.textContent = "+";
        btn.setAttribute("aria-label", "Add " + p.n + " to your budget");
        btn.setAttribute("aria-expanded", String(openRow === p.n));
        btn.addEventListener("click", function () {
          openRow = openRow === p.n ? null : p.n;
          renderList();
        });
      }
      head.appendChild(btn);
      row.appendChild(head);
      if (openRow === p.n && inCart(p.n) === -1) row.appendChild(detailFor(p));
      return row;
    }

    function renderList() {
      listEl.innerHTML = "";
      var items = BP.filter(function (p) { return bCat === "All" || p.c.indexOf(bCat) > -1; });
      items.sort(function (a, b) { return a.n.toLowerCase() < b.n.toLowerCase() ? -1 : 1; });
      var hasTrophy = false;
      items.forEach(function (p) {
        if (p.t) hasTrophy = true;
        listEl.appendChild(rowFor(p));
      });
      legendEl.hidden = !hasTrophy;
    }
    ...
    function updateTotals() {
      ...
      var er = document.getElementById("canswBEarnRow");
      er.hidden = !hasEarn;
    ...
    function openBuilder(scrollTo) {
      ...
      if (!rendered) {
        ...
        if (BCATS[sel]) bCat = sel;
        renderCats();
        renderList();
        renderBudgetItems();
        updateTotals();
      }
      if (scrollTo) builder.scrollIntoView({ behavior: reduceMotion ? "auto" : "smooth", block: "start" });
    }
    ...
    if (saved) { openBuilder(false); } else { gate.hidden = false; }
  })();
