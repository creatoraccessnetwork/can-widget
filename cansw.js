...
(function () {
  ...
  /* ---- email-gated budget builder (CFG.builder) ----
     Locked: phase-1 card + email gate. Unlocked (.cansw-mode-open): the hero
     transforms in place — left column gains a company dropdown + detail
     panel (category dropdown does double duty as the filter, with an added
     "All categories" option); the right card becomes the budget. Footprint
     stays identical to the phase-1 hero; long content scrolls internally. */
  if (CFG.builder) (function () {
    ...
    function renderDetail() {
      ...
      if (added) {
        ...
        note0.textContent = "In your budget. Switch plans or remove it there.";
      ...
      if (added) {
        add.disabled = true;
        add.textContent = "Added to your budget ✓";
      } else {
    ...
    function renderBudget() {
      ...
      countEl.textContent = CART.length ? CART.length + " added" : "";
    ...
    var allOpt = null;
    function ensureAllOpt() {
      if (allOpt) return;
      allOpt = document.createElement("button");
    ...
    select = function (name) {
      origSelect(name);
      if (OPEN) {
        if (allOpt) allOpt.setAttribute("aria-selected", "false");
        bCat = name;
        refreshCompanyMenu(true);
      }
    };
    ...
    if (saved) { openBuilder(); } else { gate.hidden = false; }
  })();
