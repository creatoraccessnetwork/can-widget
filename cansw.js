...
(function () {
  ...
  if (CFG.builder) (function () {
    ...
    if (!CFG.headline && !(cb && cb.headline)) titleEl.textContent = "Spend less to build your Creator business.";
    if (!CFG.lede && !(cb && cb.lede)) ledeEl.textContent = "CAN members get the best discounts anywhere on top Creator business software, products, and services.";
    ...
    function renderPreview() {
      ...
      if (bCat === ALL || !CATS[bCat]) {
        ...
        if (d.vals.length) {
          ...
          num.textContent = "Earn 10% more";
          sub2.textContent = "on the sales you're already making";
        } else {
          num.textContent = "Member-only deals";
      ...
      if (ranked.length) {
        ...
        var moreN = names.length - ranked.length;
        var moreEl = null;
        if (moreN > 0) {
        ...
        if (shownTrophy) {
          ...
          tnote.className = "cansw-trophy-note";
          tnote.textContent = "🏆 = Exclusive to CAN or the best deal this partner offers";
          previewEl.appendChild(tnote);
        }

        /* safety fit pass: drop any row clipped by the box (overflow hidden)
           so a half row never shows; keep the +N count honest. No-op when
           the box has no layout. */
