...
(function () {
  ...
  /* capped earn deals count as deal value (Avi 2026-07-28: Dorian's "up to
     $5,000" belongs in the headers). earnCap normally rides in
     cansw-data.json; this map is a safety net in case the Notion sync
     rewrites the JSON without it. Uncapped earn (ShopYourLikes, Insense)
     stays $0 per Avi 2026-07-26. */
  var EARN_CAPS = { "Dorian": 5000 };
  function capOf(p) { return p.earnCap || EARN_CAPS[p.n] || 0; }
  /* one ceiling per partner = the highest value of its deal on ANY basis
     (tracker savings range, best plan save, capped earnings) — the header,
     the top rows, and the ranking all use THIS, so the header can never sit
     below a row again (Avi 2026-07-28: Newsletter showed $1,212 over a
     $1,405 Revenews row because header used v-ranges and rows used plans) */
  function saveCeil(p) {
    var m = 0;
  ...
  function dealCeil(p) { return Math.max(saveCeil(p), capOf(p)); }
  var CATS = {};
  PARTNERS.forEach(function (p) {
    p.c.forEach(function (c) {
      if (!CATS[c]) CATS[c] = { partners: [], vals: [], earners: [], nvals: 0, ceil: 0, ceilSave: 0 };
      ...
      if (saveCeil(p) > CATS[c].ceilSave) CATS[c].ceilSave = saveCeil(p);
    });
  });
  ...
  function select(name) {
    ...
    if (d.vals.length) {
      ...
      var hi = d.ceil; /* highest deal value on any basis, never below a shown row */
      if (lo === hi) {
    ...
    } else if (d.ceil > 0 && d.earners.length) {
      /* earn-only category with a capped deal (e.g. Gaming: Dorian) */
      num.textContent = "Earn up to " + fmt(d.ceil);
      sub.textContent = "in extra earnings in your first year";
    } else {
  ...
  if (CFG.builder) (function () {
    ...
    function renderDetail() {
      ...
      if (!p) { detailEl.hidden = true; cPicker.classList.remove("cansw-fused"); return; }
      ...
      if (isEarn(p)) {
        wrap.appendChild(planRow(p, null, -1, capOf(p)
          ? "<span class='cansw-b-plan-save'>Earn up to " + fmt(capOf(p)) + "</span> in extra earnings"
    ...
    function renderPreview() {
      ...
      previewEl.innerHTML = "";
      /* no eyebrow, one short BLACK header line (Avi 2026-07-28): the
         teal-on-teal eyebrow/header/sub stack read badly and the long
         range sentence wasn't persuasive. Ceilings only — no floors. */
      var headerP = document.createElement("p");
      headerP.className = "cansw-savings-line";
      var names;
      if (bCat === ALL || !CATS[bCat]) {
        /* highest single deal value on any basis — capped earn deals count
           (Avi 2026-07-28: Dorian's $5,000, not Valim's $2,000) */
        var hi = 0;
        PARTNERS.forEach(function (p) { if (dealCeil(p) > hi) hi = dealCeil(p); });
        headerP.textContent = "Save up to " + fmt(hi) + " per discount";
        names = BP.map(function (p) { return { n: p.n, t: !!p.t }; });
      } else {
        var d = CATS[bCat];
        if (d.ceilSave > 0) {
          headerP.textContent = "Save up to " + fmt(d.ceil) + " per discount";
        } else if (d.ceil > 0) {
          /* earn-only category with a capped deal (e.g. Gaming: Dorian) */
          headerP.textContent = "Earn up to " + fmt(d.ceil);
        } else if (d.earners.length) {
          headerP.textContent = "Earn 10% more";
        } else {
          headerP.textContent = "Member-only discounts";
        }
        names = d.partners;
      }
      ...
      if (bCat === ALL || !CATS[bCat]) {
        ...
        /* sum of the view's deal ceilings — same basis as the rows/header
           (and as the tracker's combined_ceiling All total) */
        var planPartners = viewPool.filter(function (pp) { return rowVal(pp) !== null; });
        if (planPartners.length > 1) {
          var catSum = 0;
          planPartners.forEach(function (pp) { catSum += rowVal(pp); });
      ...
      function offerRow(r, onPick) {
        ...
        sv2.textContent = r.mx === null ? "Earn 10% more"
          : (isEarnRow(r.p) ? "Earn up to " + fmt(r.mx) : "Save up to " + fmt(r.mx)); /* always "up to" (Avi: consistency) */
        row.appendChild(sv2);
