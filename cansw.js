...
(function () {
  ...
  if (cb && (cb.name || cb.image)) {
    ...
    cols.parentNode.insertBefore(w, cols);
  }
  ...
  if (CFG.builder) (function () {
    ...
    var gateOptin = document.getElementById("canswGateOptin");
    ...
    var closeBtn = document.getElementById("canswBClose");
    var reopenBtn = document.getElementById("canswBReopen");
    var seeEl = document.getElementById("canswSee");
    ...
    /* sum of each partner's best plan — the "stack every deal" total */
    var stackTotal = 0;
    BP.forEach(function (p) {
      if (p.plans) {
        var mx = 0;
        p.plans.forEach(function (pl) { if (pl.save > mx) mx = pl.save; });
        stackTotal += mx;
      }
    });
    /* tracker total (numbers.json) wins so the widget matches the rest of the
       site; the self-computed sum is the fallback when that file is absent */
    var STACK_LABEL = (NUMBERS && NUMBERS.total_value) ||
      ("$" + (Math.floor(stackTotal / 100) * 100).toLocaleString("en-US") + "+");
    var bCat = ALL;
    ...
    if (!CFG.headline && !(cb && cb.headline)) titleEl.textContent = "Exclusive perks for content-driven businesses";
    ...
    if (!CFG.lede && !(cb && cb.lede)) ledeEl.textContent = "CAN members access " + STACK_LABEL +
      " in best-anywhere savings on the top events, software, resources, and professional services for Creators and solopreneurs.";
    if (seeEl) seeEl.hidden = false;
    ...
    function hintFor(p) {
      ...
      return "Save up to " + fmt(m); /* always "up to" — mixed labels read as inconsistent (Avi) */
    }
    ...
    function renderDetail() {
      ...
      if (!p) { detailEl.hidden = true; return; }
      ...
      if (isEarn(p)) {
        wrap.appendChild(planRow(p, null, -1, "<span class='cansw-b-plan-save'>Earn 10% more</span> on your current sales"));
      ...
      if (inCart(p.n) > -1 && !unlocked) {
        var note0 = document.createElement("p");
        note0.className = "cansw-b-desc";
        note0.style.margin = "8px 0 0";
        note0.textContent = "Picked ✓ — unlock your savings calculator to see your totals.";
    ...
    function render() {
      var n = CART.length;
      /* unlocked calculator grows to fit content; browse/gate keep the fixed footprint */
      ...
      if (!unlocked) {
        ...
        if (n > 0 && !dismissed) {
          /* gate state: header + masked picks + email gate; x to keep browsing */
          headEl.hidden = false;
          ...
          /* browse state (nothing picked, or gate dismissed): no header, category
             preview top-aligned, Unlock Access pinned at the bottom */
          headEl.hidden = true;
          countEl.textContent = "";
          subEl.hidden = true;
          gate.hidden = true;
          ...
          previewEl.hidden = false;
          renderPreview();
          reopenBtn.hidden = n === 0;
          if (n > 0) reopenBtn.textContent = "Unlock your savings calculator (" + n + " picked)";
          bCta.hidden = false;
    ...
    function renderPreview() {
      ...
      /* header + subheader FOLDED into one flowing headline (Avi: the
         stacked number-then-caption pair read as weird once both were
         sized like the hero's h1/lede) — one <p>, amount emphasized inline,
         context continues the same sentence at a lighter weight */
      var headerP = document.createElement("p");
      headerP.className = "cansw-savings-line";
      var amtSpan = document.createElement("span");
      amtSpan.className = "cansw-savings-amt";
      var ctxSpan = document.createElement("span");
      ctxSpan.className = "cansw-savings-ctx";
      var names;
      if (bCat === ALL || !CATS[bCat]) {
        ...
        var d = CATS[bCat];
        kick.textContent = bCat; /* no partner counts anywhere in the preview (Avi) */
        if (d.vals.length) {
          var lo = Math.min.apply(null, d.vals), hi2 = Math.max.apply(null, d.vals);
          amtSpan.textContent = lo === hi2 ? "Save up to " + fmt(hi2) : "Save " + fmt(lo) + " – " + fmt(hi2);
          ctxSpan.textContent = "in your first year, depending on the tools you pick";
      ...
      headerP.appendChild(amtSpan);
      headerP.appendChild(document.createTextNode(" "));
      headerP.appendChild(ctxSpan);
      if (kick.textContent) previewEl.appendChild(kick);
      ...
      function maxSave(pp) {
        if (!(pp.plans && pp.plans.length)) return null;
        var mx = 0;
      ...
      function offerRow(r, onPick) {
        ...
        sv2.textContent = r.mx === null ? "Earn 10% more" : "Save up to " + fmt(r.mx); /* always "up to" (Avi: consistency) */
        row.appendChild(sv2);
        row.addEventListener("click", onPick);
        return row;
      }
      var ranked = viewPool
        .filter(function (pp) { return pp.plans && pp.plans.length; })
        .map(function (pp) { return { p: pp, mx: maxSave(pp) }; })
        .sort(function (a, b) { return b.mx - a.mx; })
        /* earn-only partners fill remaining slots so no category renders an
           empty card (Affiliates was blank: ShopYourLikes has no plans) */
        .concat(viewPool
          .filter(function (pp) { return isEarn(pp) && !(pp.plans && pp.plans.length); })
      ...
      if (ranked.length) {
        ...
        tl.textContent = "Top category discounts";
        tops.appendChild(tl);
    ...
    gate.addEventListener("submit", function (ev) {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v)) {
        gateErr.textContent = "Enter a valid email to unlock your savings calculator.";
        gateErr.hidden = false;
        gateEmail.focus();
        return;
      }
      if (!gateOptin || !gateOptin.checked) {
        gateErr.textContent = "You need to agree to receive the newsletter to unlock your savings calculator.";
        gateErr.hidden = false;
        if (gateOptin) gateOptin.focus();
        return;
      }
      gateErr.hidden = true;
    ...
    closeBtn.addEventListener("click", function () { dismissed = true; render(); });
    reopenBtn.addEventListener("click", function () { dismissed = false; render(); try { gateEmail.focus(); } catch (e) {} });
  ...
  /* data: cansw-data.json next to this script; baked fallback on any failure.
     ...
     partner_count + total_value — written weekly by the can-deal-numbers
