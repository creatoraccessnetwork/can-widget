...
def render(text, nums):
    ...
    s = re.sub(r"\{(total|count|median|starter|value|trophy)\}", lambda m: "🏆" if m.group(1) == "trophy" else str(nums.get(m.group(1), m.group(0))), text)
...
def plain(key, nums):
    ...
    assert key in COPY, key
    return re.sub(r"\{(total|count|median|starter|value|trophy)\}", lambda m: "🏆" if m.group(1) == "trophy" else str(nums.get(m.group(1), m.group(0))), COPY[key])
