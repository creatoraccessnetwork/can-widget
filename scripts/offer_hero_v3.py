...
What changes on each page (hero section 1787370000000 only; everything else untouched):
  ...
  * The hero block also carries the shared CSS for the page, so the CSS is regenerated from offer_page_lib
    (the can-offer-page skill's build_offer_page.py with the figures read from numbers.json).
...
def hero_html(cfg, style):
    if cfg.get("partner_logo"):
        ...
        lock = ""
    return style + FORM_CSS + """<div class="canv2 hero-in hero" id="top">
  %(lock)s
...
def build(cfg):
    ...
    style = lib.css(white, pattern)
    sec = lib.section("Hero (v2)", [(IDS["hero"] + "_0", hero_html(cfg, style))], top=72, bottom=64, mtop=56, mbottom=40)
