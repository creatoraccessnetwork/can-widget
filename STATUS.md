...
### 2026-09-13 - Homepage copy from a Google Sheet (Claude Code) - Kajabi write PENDING
- Avi wants to edit homepage copy himself and have it go live. Chosen path (his call): a Google Sheet the page reads directly. Built:
  - Sheet "CAN Homepage Copy" (id `1Yu0udf_dt9qTi9INE3VFJlnrOpKCdAGIPC2tBdLClJk`, owner avi@creatoraccessnetwork.com, columns Section | Where | Text | Key, 120 rows + 3 HOW TO rows). **Avi must share it as "anyone with the link: viewer"** or `can-copy.js` gets a 401 and the baked text stays. Earlier copies (17w8..., 1q0G..., 1t4u...) are in Drive trash: the CSV import mangled the trophy emoji, so copy now uses `{trophy}`.
  - `can-copy.js` (Pages): reads the sheet via gviz JSON (JSONP fallback), fills `[data-copy]` slots + `data-copy-attr`, selector map for JS-rendered nodes (quiz band, wins header), `window.CANCOPY_MAP` + `cancopy` event for the quiz, sessionStorage cache, numbers.json for `{total}` `{count}`, `{median}` `{starter}` from config, `{trophy}`; markup `**bold**` `[[teal]]` `[text](url)`; escapes HTML; `?copy=off` disables. Verified in the local harness with a seeded map (all slots, quiz re-render, escaping).
  - `copy.json` (snapshot of the sheet, seeded from live copy), `scripts/copy_lib.py` (render/slot, same rules as the JS), `scripts/pull_copy.py` (sheet CSV -> copy.json; run before a rebuild), `copy_sheet.json` (sheet id). `build_home_v3.py` now renders hero / wins / how / founder / FAQ / pricing from copy.json with `data-copy` slots (verified: only attributes differ from the previous build, plus `<b class="lead">` for bold and a span around the hero arrow line). `can-quiz.js` reads `quiz.*` keys via `cp()` and re-renders on `cancopy`.
- **PENDING:** the Kajabi write of the six blocks (hero 08, wins 10, how 04, founder 02, FAQ 05, pricing 06). Payload ready at `scripts/payloads/home-v3-copy-slots-write.json` (pass its `sections` to update_theme_content on theme 2164431783). The Kajabi MCP in this session started rejecting every object argument after the desktop app restart ("settings of type string"); do the write from a fresh session, then verify the served HTML contains `data-copy=` and that the sheet loads (no 401) once Avi has shared it.
- Note for can-deal-numbers: hero stats and pricing bullet 1 now carry `{total}`/`{count}` placeholders resolved from numbers.json in the browser, so updating numbers.json + a rebuild is enough; the sync no longer needs to edit those strings in Kajabi.
