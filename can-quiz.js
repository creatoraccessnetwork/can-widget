 ...
 * v1.2, 2026-09-13: strings read the "CAN Homepage Copy" sheet via window.CANCOPY_MAP (cp(); can-copy.js) and re-render on "cancopy".
...
"@media (prefers-reduced-motion:reduce){.canq .bar i{transition:none}.canq .panel{animation:none}.canq.embed .opt,.canq.embed .opt:hover{transition:none;transform:none}}";
  ...
  function finishStarter() {
    ...
    state.done = true; body.innerHTML = '<div class="done list"><div class="q">' + esc(cp("quiz.done.title", "Here's what we can help you build.")) + '</div>' +
      '<p>' + esc(cp("quiz.done.text", "We have all the discounts you need to launch the projects below with the best tools and services for the least money. Check out what we can help you build, or come back when you're ready for your next project.")) + '</p>' +
      list +
      '<div class="starter"><p class="fine">' + esc(cp("quiz.done.starter", "Or start with the free Starter Set: " + STARTER_COUNT + " discounts worth " + STARTER_VALUE + ". No card required.")) + '</p><a class="btn" href="' + esc(STARTER) + '">' + esc(cp("quiz.done.button", "Get the Starter Set")) + '</a></div>' +
      '<div class="nav"><button type="button" class="back">' + esc(cp("quiz.back", "← Back")) + '</button><span></span></div></div>';
